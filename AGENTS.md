...
| `coupon_types`, `risk_levels` | list | CSV-списки |
...
**Скринер:** фильтры и сортировка — только через query params `GET /bonds/` (новый запрос при изменении); infinite scroll по 50 бумаг. `risk_profile`, `rate_scenario` — как раньше. `max_days` / `min_volume_rub` из `GET /config/` — начальные значения UI, не серверные константы. Портфель: `GET /bonds/by-isins?risk_profile=<portfolio.risk_profile>`.
