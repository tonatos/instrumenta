...
def get_bond_coupon_schedule(
    ...
    try:
        ...
        if isinstance(exc, UnauthenticatedError):
            logger.warning("T-Invest auth failed for coupon schedule (figi=%s)", figi)
            return []
...
# ── Trading helpers (используются в режиме торговли) ─────────────────────────


@dataclass
class TradeAvailability:
    """Флаги доступности торговли инструментом из ``bond_by``.

    Используется перед `post_limit_order` — если бумага не торгуется
    через API или не доступна для BUY/SELL, заявка отклонится с
    непонятной ошибкой; лучше отдать понятный warning в UI.
    """
...
def resolve_figi_for_isin(token: str, isin: str) -> str | None:
    """Найти FIGI облигации по ISIN через `instruments.find_instrument`.

    Используется при подготовке `PendingOperation` — FIGI нужен и для
    `post_limit_order`, и для матчинга позиций по `account_snapshot`.

    Args:
        token: T-Invest API токен (read-only достаточно).
        isin: ISIN облигации (формат ``RU000A...``).

    Returns:
        FIGI первой совпавшей облигации, или ``None`` если бумага не
        найдена / API недоступен.
    """
    try:
    ...
    try:
        ...
        logger.exception("find_instrument failed for ISIN=%s", isin)
        return None

    matches = [
        instrument
        for instrument in resp.instruments
        if (instrument.isin or "").upper() == isin.upper()
    ]
    if matches:
        for instrument in matches:
            if bool(getattr(instrument, "api_trade_available_flag", False)):
                return instrument.figi
        return matches[0].figi
    # Fallback: если точного матча по ISIN нет, но есть единственный
    # результат — возвращаем его. Это случается, когда ISIN в API
    # подаётся через ticker-поиск.
    if len(resp.instruments) == 1:
        return resp.instruments[0].figi
    return None


def check_trade_available(
    token: str,
    figi: str,
    *,
    instrument_uid: str | None = None,
) -> TradeAvailability | None:
    """Проверить флаги доступности торговли облигации.

    Сначала ищет по ``instrument_uid`` (как в снимке портфеля), затем по FIGI.
    Возвращает ``None`` если бумага не найдена / API упал.
    """
    try:
    ...
    for id_type, id_value in lookups:
        try:
            ...
            logger.exception("bond_by failed for %s=%s", id_type, id_value)
            continue

        bond = resp.instrument
        lot_size = int(getattr(bond, "lot", 1) or 1)
        return TradeAvailability(
            api_trade_available_flag=bool(bond.api_trade_available_flag),
            buy_available_flag=bool(bond.buy_available_flag),
            sell_available_flag=bool(bond.sell_available_flag),
            figi=bond.figi or figi,
            instrument_uid=bond.uid or instrument_uid or "",
            lot_size=max(1, lot_size),
        )
    return None


def ensure_order_instrument(
    token: str,
    *,
    figi: str | None = None,
    instrument_uid: str | None = None,
    ...
    """Разрешить инструмент для заявки и проверить доступность торговли через API.

    Raises:
        ValueError: Инструмент не найден.
        TradingNotAvailableError: Торговля через API запрещена или направление недоступно.
    """
    from bond_monitor.infrastructure.tinvest.trading_client import TradingNotAvailableError

    resolved_figi = figi or ""
    if not resolved_figi and isin:
        resolved_figi = resolve_figi_for_isin(token, isin) or ""

    trade = check_trade_available(token, resolved_figi, instrument_uid=instrument_uid)
    if trade is None:
        label = isin or instrument_uid or figi or "?"
        raise ValueError(f"Инструмент не найден в T-Invest API: {label}")

    if not trade.api_trade_available_flag:
        raise TradingNotAvailableError(
            f"Облигация {isin or trade.figi} недоступна для торговли через API "
            f"(api_trade_available_flag=false). Выберите другую бумагу или "
            f"оформите заявку вручную в приложении брокера."
        )
...
def get_last_price_pct(token: str, figi: str) -> float | None:
    """Последняя цена облигации в **% от номинала**.

    Для лимитных заявок UI предзаполняет цену как ``last_price × (1 ± buffer)``.
    Возвращает ``None`` если цена недоступна (нет торгов в этой сессии).

    Для облигаций T-Invest API возвращает цену в той же шкале, что и в
    `BondRecord.last_price` — проценты от номинала.
    """
    try:
        from t_tech.invest import Client
    except ImportError:
    ...
    try:
        ...
        logger.exception("get_last_prices failed for figi=%s", figi)
        return None

    for entry in resp.last_prices:
        price = entry.price
        if price is None:
            continue
        units = int(getattr(price, "units", 0))
        nano = int(getattr(price, "nano", 0))
