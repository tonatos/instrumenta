...
class PortfolioPlan:
    ...
    * ``final_portfolio_value_rub`` — сумма этих двух, удобная цифра
    ...
    # Средневзвешенная дюрация Маколея (годы) ТЕКУЩИХ позиций, взвешенная
    # по сумме покупки. Метрика процентного риска: чем выше — тем сильнее
    # переоценка тела при изменении ключевой ставки. None — нет данных о
    # дюрации ни по одной позиции.
