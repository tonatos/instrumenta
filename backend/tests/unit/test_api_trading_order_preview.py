...
def _snapshot(*, money_rub: float = 150_000.0) -> AccountSnapshot:
...
async def test_preview_pending_operation_returns_broker_total_with_nkd() -> None:
    ...
    with (
        ...
        patch(
            "bond_monitor.application.trading.trading_service.preview_order_price",
...
def test_preview_endpoint_returns_order_preview_response() -> None:
    with TestClient(app=create_app()) as client, patch(
        "bond_monitor.interfaces.api.controllers.TradingService.preview_pending_operation",
