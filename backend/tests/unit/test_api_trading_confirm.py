...
def _snapshot(account_id: str = "acc-clean") -> AccountSnapshot:
...
def _attach(client: TestClient, pid: str) -> str:
    ...
    with (
        patch(
            ...
            return_value=_snapshot(account_id),
        ),
        patch(
        ...
        patch(
            "bond_monitor.application.trading.trading_service.ensure_order_instrument",
...
def test_confirm_pending_submits_order_and_returns_in_progress() -> None:
    with _portfolio_client() as (client, pid):
        ...
        with (
            ...
            patch(
                "bond_monitor.application.trading.trading_service.get_order_state",
            ...
            patch(
                "bond_monitor.application.trading.trading_service.post_limit_order",
...
def test_refresh_trade_record_states_updates_amounts_from_broker() -> None:
    ...
    with patch(
        ...
        updated = service._refresh_trade_record_states(portfolio, "token")
